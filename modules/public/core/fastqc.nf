...
process FASTQC {
    ...
    function process_data {
        ...
        printf "\${sampleId},fastqc,q30_\${readNumLower},\$(echo \$ave_q30)\n" >> "\${COUNTFILE}"

        # Write input read counts to file
        printf "${sampleId},fastqc,read_count_\${readNumLower},\$(echo \$READCOUNTS)\n" >> "\${COUNTFILE}"
    }
    ...
    EXPECTED_INPUT_COUNT=14

    # Check that we have expected number of sample input files
    check_input_files "FASTQC" "${sampleId}" "\$EXPECTED_INPUT_COUNT"

    # Create expected output files
    outputs=("${sampleId}_R1_001.ds.complete_fastqc.zip"
    "${sampleId}_R2_001.ds.complete_fastqc.zip" "${sampleId}_input_read_counts.csv"
    "${sampleId}_R1_quality_scores.csv" "${sampleId}_R2_quality_scores.csv"
    "${sampleId}_R1_sequence_traces.csv" "${sampleId}_R2_sequence_traces.csv"
    "${sampleId}_R1_001.ds.complete_fastqc.html"
    "${sampleId}_R2_001.ds.complete_fastqc.html")
    ...
    param_names=("params.options.resultsDir" "fastq" "sampleId" "fastq[0]" "task.cpus"
    "fastq[1]" "workflow.manifest.version" "task.memory.toGiga()" "params.options.fastqcDs")

    parameters=("${params.options.resultsDir}" "${fastq}" "${sampleId}" "${fastq[0]}"
    "${task.cpus}" "${fastq[1]}" "${workflow.manifest.version}" "${task.memory.toGiga()}"
    "${params.options.fastqcDs}")
    ...
    do
        ...
        check_params "FASTQC" "\$param_name" "\$parameter"
    done
    """
}
