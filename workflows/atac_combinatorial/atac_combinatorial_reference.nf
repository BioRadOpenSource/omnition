...
workflow ATAC_COMBINATORIAL_REFERENCE {
    take:
    ...
    if (!params.catac.reference.blocklist) {
        ...
        block_count = Core.countElement(params.catac.reference.blocklist)
        if (block_count == 1) {
            // If one blocklist is supplied format it
            ch_blocklist_pre = Channel.fromPath(params.catac.reference.blocklist).first()
            FORMAT_BLOCKLIST(
                ch_blocklist_pre,
                ch_images_pulled
           )
            ...
            ch_blocklist_pre = Channel.fromPath(params.catac.reference.blocklist)
            COMBINE_BLOCKLISTS(
