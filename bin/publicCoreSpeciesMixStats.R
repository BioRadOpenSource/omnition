...
# Create the metric summary formatted output
df$sample <- prefix
df$process <- "crosstalk"
pivot_df <- melt(df, id = c("sample", "process"), variable.name = "metric")
pivot_df$metric <- tolower(pivot_df$metric)
# string replace in metric

if (assay == "rna") {
  pivot_df$metric <- gsub(
    "measurable_crosstalk",
    "observed_mixed_species_multiplets",
    pivot_df$metric
  )
  pivot_df$metric <- gsub(
    "estimated_total_crosstalk",
    "estimated_total_multiplets",
    pivot_df$metric
  )
}
