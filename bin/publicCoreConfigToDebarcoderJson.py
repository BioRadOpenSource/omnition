...
options.add_argument(
    ...
    "--max-dist",
    required=True,
    help="Maximum allowable distance (number of errors tolerated).",
)
options.add_argument("-r", "--read", required=True, help="Read where TIs are present.")
options.add_argument(
    "-t",
    "--i7AsTi",
    required=True,
...
options.add_argument(
    "-c", "--config", required=True, help="Existing atac config json for DEAD."
)
                ...
                if len(data[1]) != TIlen:
                    if TIlen == 0:
                        ...
                        print(
                            ...
                            " Please check pipeline config file."
                        )
                        exit(1)
            data = f.readline()
...
# Create the JSON file from the settings and indexes
def editR1JSON(indexlist, maxdist, outfile, r1json):
    with open(r1json) as f:
...
if __name__ == "__main__":
    ...
    if args.read == "r2" or args.i7AsTi == "true":
        ...
        editR1JSON(indexlist, args.max_dist, outfile, args.config)
