...
# create output for genes per cell at the 50k level for the single-cell dashboard
if (mean_read_pairs_per_cell >= 50000) {
    # Calculate subsampling intervals for downsampling gene counts to include 50k
    mean_depth_per_cell_50k <- unique(sort(append(mean_depth_per_cell, 50000)))
    gene_count_50k <- round((50000 / mean_read_pairs_per_cell) * total_gene_counts)
    subs_intervals_downsample_50k <- unique(sort(append(subsample_intervals_downsample,
        gene_count_50k)))
    subsample_stat_table_50k <- lapply(
        subs_intervals_downsample_50k,
        FUN = subsampler,
        feature_dt = feature_dt,
        total_gene_counts = total_gene_counts,
        interval_form = interval_form
    ) %>% rbindlist(.)
    output_file_50k <- cbind(
    mean_depth_per_cell_50k,
    subsample_stat_table_50k[, c("median_obs_per_cell")]
    )
    if (is.null(species_id)) {
        output_file_name_50k <- paste0(sample_id, "_seq_sat_50k.csv")
    } else {
        output_file_name_50k <- paste0(sample_id, ".", species_id,
            "_seq_sat_50k.csv")
    }
    # Write out saturation curve coordinates for 50k
    fwrite(output_file_50k, output_file_name_50k)
}
