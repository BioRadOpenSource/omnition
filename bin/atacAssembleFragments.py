...
@click.option(
    "--input", "-i", help="Path to an indexed BAM file with duplicates marked."
)
@click.option(
    "--sample",
    "-s",
...
def assemble_fragments(
    ...
    input_count, output_count = write_to_output_file(
        bamfile, fragment_path, mapping_quality, insert_size, chromosome_length
    )
    write_to_readcount_file(sample, chromosome, input_count, output_count, verbose)
...
def is_read1_start_5prime_of_read2_start(read_dict, mapping_quality, insert_size):
    ...
    return (
        read_dict["r1_ref_start"] <= read_dict["r2_ref_start"]
        and min(read_dict["r1_map_qual"], read_dict["r2_map_qual"]) >= mapping_quality
        and (read_dict["r2_ref_end"] - read_dict["r1_ref_start"]) <= insert_size
    )
...
def is_read2_start_5prime_of_read1_start(read_dict, mapping_quality, insert_size):
    ...
    return (
        read_dict["r1_ref_start"] > read_dict["r2_ref_start"]
        and min(read_dict["r1_map_qual"], read_dict["r2_map_qual"]) >= mapping_quality
        and (read_dict["r1_ref_end"] - read_dict["r2_ref_start"]) <= insert_size
    )
...
def get_read_count_list(sample, chromosome, input_count, output_count):
    """get read count list"""
...
def write_to_readcount_file(
    sample: str, chromosome: str, input_count: int, output_count: int, verbose: bool
):
    ...
    read_count_list = get_read_count_list(sample, chromosome, input_count, output_count)
...
def get_reference_values_by_orientation(
    orientation: str, read_dict, chromosome_length: int
):
...
def update_paired_read_dict(read_name: str, read_dict, read):
    ...
    if read.is_read1:
        ...
        read_dict[read_name]["support"] = (
            read.get_tag("DS") if read.has_tag("DS") else 1
        )
    else:
...
def bedpe_generator(bamfile, input_count_int):
