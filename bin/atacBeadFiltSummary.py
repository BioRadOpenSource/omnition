...
# Take the median from the Unique fragment threshold at knee from all TIs associated with the sample
...
# Aggregate all above knee beads from all TIs and and find the median accross all the above knee beads
...
# Open the barcode quant file and calculate the median fragments in beads above the threshold
...
# Open the QC stats file and count the total cells and calculate the median fragments per cell
...
if __name__ == "__main__":
    ...
    F.write("FastqTI,UniqFragThresh,MedianBeadFrags,TotalCells,TotalReadPairsPerTotalCells,MedianCellFrags\n")
    ...
    sample_med_frag_per_bead = readSampleBarcodeQuant(args.input_directory,sample_thresh)
    sample_cell_count, sample_med_frag_per_cell = readSampleQCStats(args.input_directory)
    sample_read_count = readSampleTIcounts(args.sample_id, args.input_directory)
    sample_stats = [args.sample_id,sample_thresh,sample_med_frag_per_bead,sample_cell_count,round(sample_read_count/sample_cell_count,1),sample_med_frag_per_cell]
    formatted_sample_stats = [str(x) for x in sample_stats]

    with open(args.input_directory + args.sample_id + ".sampleBeadFiltSummary.csv", "w") as outfile:
        outfile.write("Sample,UniqFragThresh,MedianBeadFrags,TotalCells,TotalReadPairsPerTotalCells,MedianCellFrags\n")
