...
parser$add_argument("-bl", "--bead_lambda", type = "double", default = 3, help = "Expected gel bead lambda.")
parser$add_argument("-ed", "--estimated_droplets", type = "integer", default = 100000, help = "Estimated number of droplets.")
...
bead_lambda <- parser$bead_lambda
estimated_droplets <- parser$estimated_droplets
...
calc_poiss <- function(comps, events, lambda, min_umi = 1) {
    # ===================================
    ...
    # If min_umi == 10, filter out cell barcodes with less than 10 UMIs
    if (min_umi > 1) {
        comps <- comps[umi.bead >= min_umi]
        comps <- comps[, (events) := .N, by = cell_barcode]
        # Make sure that we didn't filter out everything
        if (nrow(comps) == 0) {
            warn_msg <- paste0(
                "No cell barcodes with 10 or more UMIs detected."
            )
            warning(warn_msg)
            return(NULL)
        }
...
calc_lambdas <- function(all_beads, estimated_droplets, bead_lambda, prefix) {
    # ===================================
    # Calculate observed lambda based on average beads per droplet
    # Calculate lambda based on observed beads and estimated droplets
    # Create summary table for the metric summary
    # ===================================

    all_cells <- all_beads[, .(cell_barcode, n_beads)] %>%
        unique(.)

    obs_lambda <- all_cells[, n_beads] %>% mean()

    total_beads <- all_cells[, n_beads] %>% sum()
    calc_lambda <- total_beads / estimated_droplets

    lambda_summary <- data.table(
        sample = prefix,
        process = "bead_merging",
        metric = c(
            "user_input_lambda",
            "observed_all_bead_mean_lambda",
            "calculated_all_bead_lambda"
        ),
        value = c(bead_lambda, obs_lambda, calc_lambda)
    )
...
# Calculate Poisson distributions with a minimum of 10 UMIs
poiss_dist_list_2 <- parallel::mclapply(inp_list,
        FUN = calc_poiss,
        events = "n_beads",
        lambda = bead_lambda,
        min_umi = 10
...
out_file5 <- paste0(out_prefix,
    ".min_10_umi_above_knee_beads_per_partition_poisson.csv")
out_file6 <- paste0(out_prefix, ".min_10_umi_above_knee_beads_per_partition.csv")

# raw, unformatted output
out_file7 <- paste0(out_prefix, ".above_knee_beads_per_partition_raw_poisson.csv")

out_file8 <- paste0(out_prefix, ".gel_bead_lambda_summary.csv")
...
# Check if poiss_dist_list_2$abv_knee_beads is NULL
if (!is.null(poiss_dist_list_2$abv_knee_beads)) {
    poiss_dist_list_2$abv_knee_beads %>%
        fmt_dist_tbl(., count_bins = count_bins) %>%
        fwrite(., file = out_file5)

    poiss_dist_list_2$abv_knee_beads[, c("Count", "Observed")] %>%
        setNames(., c("drop_size", "Frequency")) %>%
        fwrite(., file = out_file6)
}

poiss_dist_list$abv_knee_beads %>%
    fwrite(., file = out_file7)
