...
parser$add_argument(
  "-b",
  "--blocklist",
  help = "Blocklist .bed file for genome used."
)
parser$add_argument(
  "-w",
  "--peak_width",
...
makePeaksDF <-
  function(summit_file,
    ...
    peakdf <- summit_file[summit_file$V5 > -1 * log10(fdr_threshold), ]
    # Import sizes and remove chromosomes
    sizedf <- read.table(chrom_sizes_file, stringsAsFactors = FALSE)
    ...
    peaks <- trim(peaks) # does the actual trimming, all out of bounds
    # ranges will now start or end at the end of the contig
    bdf <- data.frame(fread(input = blocklist_bed_file, header = FALSE))
    if (nrow(bdf > 0)) {
      bg <-
...
# Create a summary file with the number of peaks called
...
write.csv(
  summary_df,
  file = paste0(args$output_directory, "/", args$name, ".peak_calling_summary.csv"),
  row.names = FALSE,
  quote = FALSE
)
