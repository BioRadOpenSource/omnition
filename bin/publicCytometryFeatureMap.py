...
parser.add_argument(
    "FeatureRefCSV", help="path to csv file with whitelist Feature Barcodes"
)
parser.add_argument(
    "--t2g",
    help="path to output t2g file, default ./FeaturesMismatch.t2g",
    default="./FeaturesMismatch.t2g",
    type=str,
)
parser.add_argument(
    "--fa",
    help="path to output fasta file, default ./FeaturesMismatch.fa",
    default="./FeaturesMismatch.fa",
    type=str,
)
parser.add_argument(
    "--header",
    help="include this flag if FeatureRefCSV has a header, omit if not",
    action="store_true",
)
parser.add_argument(
    "-q", "--quiet", help="don't print processed Feature Barcodes", action="store_true"
)
...
def make_mismatch_map(FeatureDict):
    ...
    for item in FeatureDict:
        ...
        if counter == 0:
            ...
            if not args.quiet:
                print("Read " + str(len(FeatureDict)) + " Feature Barcodes:\n")
            counter += 1
        if not args.quiet:
            print(name)
            print(seq)
...
def kite_mismatch_maps(FeatureDict, mismatch_t2g_path, mismatch_fasta_path):
    write_mismatch_map(
        make_mismatch_map(FeatureDict), mismatch_t2g_path, mismatch_fasta_path
    )
    if not args.quiet:
