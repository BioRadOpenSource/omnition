...
def main(input_dir):
    ...
    if not files:
        print("No .rna.h5ad or .adt.h5ad files found in input directory.")
        return
    ...
    # Determine output extension based on input files
    if all(f.endswith("_rna.h5ad") for f in files):
        out_ext = "_rna.h5ad"
    elif all(f.endswith("_cyto.h5ad") for f in files):
        out_ext = "_cyto.h5ad"
    else:
        out_ext = ".h5ad"  # fallback if mixed

    # Write merged h5ad
    all_samples.write(f"all_samples{out_ext}", compression="gzip")
...
def gather_h5ad_files(input_dir):
    ...
    """
    # Glob all .rna.h5ad and .adt.h5ad files
    files = [
...
def merge_h5ad_samples(dic, samples):
    ...
    all_samples = all_samples.concatenate(
        ...
        index_unique=None,  # Prevents appending sample info to barcode index
    )
    return all_samples
