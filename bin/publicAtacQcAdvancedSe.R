...
if (peakFile != "none") {
  ...
  saveRDS(SE, paste0(sampleId, ".SE.rds"))
}

# Either way, export QC stats
write.table(
  qcStats,
  file = paste0(sampleId, ".QCstats.csv"),
  quote = FALSE,
  row.names = FALSE,
  col.names = TRUE,
  sep = ","
...
median_uniq_nuc_frag_per_cell <- median(qcStats$uniqueNuclearFrags)
median_est_lib_size_per_cell <- median(qcStats$librarySize)
...
summary_data <- data.frame(
  ...
  metric = c(
    "median_unique_nuclear_fragments_per_cell",
    "median_estimated_library_size_per_cell"
  ),
  value = c(
    median_uniq_nuc_frag_per_cell,
    median_est_lib_size_per_cell
  )
)
...
write.csv(
  summary_data,
  file = paste0(sampleId, ".qc_metrics.csv"),
  row.names = FALSE,
  quote = FALSE
)
