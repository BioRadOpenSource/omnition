...
def main(edge_file, barcodes, sample_id):
    ...
    # write output
    filtered_edgelist.write_csv(f"{sample_id}_edges.tsv", has_header=False, separator="\t")
...
def read_allowlist(allowlist_file, merged):
    ...
    if merged:
        allowlist = pl.read_csv(
            allowlist_file, separator="\t", has_header=False, new_columns=["droplet"]
        ).with_row_count(name="rank", offset=1)
    else:
        allowlist = pl.read_csv(
            allowlist_file, separator="\t", has_header=False, new_columns=["bead"]
        ).with_row_count(name="rank", offset=1)
    return allowlist
