# unit tests to run for publicCoreCalcBeadPlots.R
...
# load test file
test_input <- "files/test_publicRnaWorkflow.bead_summary.csv"

# Read lines and include only dependency and functions
lines <- scan("../../../../bin/publicCoreCalcBeadPlots.R", what = character(), sep = "\n")
...
### Test calc_lambdas ###
# ===================================
# Take in all_beads data.table from prep_input
# Calculate average lambda based on mean of n_beads
# Calculate lambda based on total n_beads and estimated droplets
# Create a data table with the two lambdas and the input lambda
# Output data.table with 4 columns

# Prep test cases
estimated_droplets <- 100
input_lambda <- 3
prefix <- "test"

mean_lambda <- 2
estimated_lambda <- 0.12

# Get function output
lambda_output <- calc_lambdas(prep_output$all_beads, estimated_droplets, input_lambda, prefix)

# Test
test_that("Pass - calc_lambdas", {
    # Class checks
    lambda_output %>%
        is.data.table(.) %>%
        expect_true(.)
    ...
    expect_equal(ncol(lambda_output), 4)

    # Check that the output has the correct number of rows
    expect_equal(nrow(lambda_output), 3)

    # Check that the output has the correct column names
    expect_equal(colnames(lambda_output), c("sample", "process", "metric", "value"))

    # Check that the output has the correct values
    expect_equal(lambda_output[metric == "user_input_lambda", value], input_lambda)
    expect_equal(round(lambda_output[metric == "observed_all_bead_mean_lambda", value],2), mean_lambda)
    expect_equal(lambda_output[metric == "calculated_all_bead_lambda", value], estimated_lambda)
})
