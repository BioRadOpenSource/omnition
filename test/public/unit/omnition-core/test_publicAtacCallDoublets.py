...
spec = importlib.util.spec_from_file_location("merging", "bin/publicAtacCallDoublets.py")
...
@pytest.fixture
def constants():
    pytest.quant_file = "test/public/unit/omnition-core/files/test_publicAtacCallDoublets_barcodeQuantSimple.csv"
    pytest.allowlist_file = "test/public/unit/omnition-core/files/test_publicAtacCallDoublets_allowlist.csv"
    pytest.ti_len = 6
    pytest.csv_dir = "test/public/unit/omnition-core/files/"
    pytest.name = "DemoAtacCombinatorial_S1"
    pytest.one_to_one = False
    pytest.barcoded_tn5 = True
    pytest.params_prefix = "test/public/unit/omnition-core/files/test_publicAtacCallDoublets"
    pytest.params_suffix = ".deconvolutionParams.orig.csv"
...
def test_read_n_bc_file(constants):
    allowlist_bc = mod.read_hq_bc_file(pytest.allowlist_file)
    unfiltered_df, quantification_df = mod.read_n_bc_file(pytest.quant_file, allowlist_bc['bc'].to_list())
    assert isinstance(unfiltered_df, pl.DataFrame)
    assert isinstance(quantification_df, pl.DataFrame)
    assert unfiltered_df.shape[0] == 14849
    assert unfiltered_df.shape[1] == 2
    assert quantification_df.shape[0] == 2000
    assert quantification_df.shape[1] == 2

def test_create_count_df(constants):
    allowlist_bc = mod.read_hq_bc_file(pytest.allowlist_file)
    _unfiltered_df, quantification_df = mod.read_n_bc_file(pytest.quant_file, allowlist_bc['bc'].to_list())
...
def test_create_implicated_df(constants):
    ...
    allowlist_bc = mod.read_hq_bc_file(pytest.allowlist_file)
    _unfiltered_df, quantification_df = mod.read_n_bc_file(pytest.quant_file, allowlist_bc['bc'].to_list())
    count_df = mod.create_count_df(quantification_df.lazy())
...
def test_group_beads(constants):
    ...
    implicated_df =     implicated_df = implicated_df.with_columns(
        (pl.col("jaccard_frag") > jaccard_min_frag).alias("merged")
    )
    barcode_filtered_df = implicated_df.filter(pl.col("merged"))
    barcode_translate_df = quantification_df.with_columns(pl.lit("").alias("droplet_barcode"))
...
def test_create_updated_deconvolution_params(constants):
    overlap_df = mod.load_overlap_df(pytest.csv_dir)
    overlap_df = mod.substr_right(overlap_df, "barc1", int(pytest.ti_len))
    overlap_df = mod.substr_right(overlap_df, "barc2", int(pytest.ti_len))
    overlap_df = overlap_df.filter(pl.col("match_barc1") == pl.col("match_barc2")).select([pl.col("barc1"), pl.col("barc2"), pl.col("n_both")])
    ...
    implicated_df = mod.create_implicated_df(overlap_df, count_df)
    jaccard_results = mod.get_density_threshold(implicated_df.select([pl.col("jaccard_frag")]).head(1000000),"jaccard", logTransform = True)
    bead_threshold, updated_params_df = mod.create_updated_deconvolution_params(
        jaccard_results[1],
        jaccard_results[0],
        pytest.params_prefix,
        pytest.params_suffix,
    )
    assert bead_threshold == 54.0
    assert updated_params_df.shape[0] == 5
    assert updated_params_df.shape[1] == 2


def test_compile_deconvolution_metrics(constants):
    overlap_df = mod.load_overlap_df(pytest.csv_dir)
    overlap_df = mod.substr_right(overlap_df, "barc1", int(pytest.ti_len))
    overlap_df = mod.substr_right(overlap_df, "barc2", int(pytest.ti_len))
    overlap_df = overlap_df.filter(pl.col("match_barc1") == pl.col("match_barc2")).select([pl.col("barc1"), pl.col("barc2"), pl.col("n_both")])
    ...
    implicated_df = implicated_df.with_columns(
        (pl.col("jaccard_frag") > jaccard_results[1]).alias("merged")
    )
    barcode_filtered_df = implicated_df.filter(pl.col("merged"))
    barcode_translate_df = quantification_df.with_columns(pl.lit("").alias("droplet_barcode"))
    barcode_translate_df = mod.group_beads(quantification_df,
                            barcode_filtered_df, barcode_translate_df, pytest.one_to_one, pytest.name, int(pytest.ti_len), pytest.barcoded_tn5)
    metrics_df = mod.compile_deconvolution_metrics(
        pytest.name,
        unfiltered_df,
        quantification_df,
        bead_threshold,
    ...
    assert metrics_df.shape[0] == 7
    assert metrics_df.shape[1] == 4
    assert metrics_df.filter(pl.col("metric") == "median_frags_per_above_knee_bead")["value"].to_list()[0] == 19.0
