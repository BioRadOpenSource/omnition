...
spec = importlib.util.spec_from_file_location("correcting", "bin/publicRnaFilterBeads.py")
...
def test_filter_edgelist():
    edgelist = mod.read_edgelist(
        "test/public/unit/omnition-core/files/test_filter_edge_file.tsv"
    ).collect()
    allowlist = mod.read_allowlist(
        "test/public/unit/omnition-core/files/test_barcode_file.csv", True
    )
