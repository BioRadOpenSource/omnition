...
spec = importlib.util.spec_from_file_location("tag_features", "bin/publicRnaGeneTagger.py")
...
def test_tagging_strategy_exclude_introns():
	...
	feature_tag = "XF"
	assignment_tag = "XS"
	bam = pysam.AlignmentFile("test/public/unit/omnition-dbg/files/test_rna_tag_features.bam")
	Itr = bam.fetch()
	reads = list(Itr)
	for read in reads:
